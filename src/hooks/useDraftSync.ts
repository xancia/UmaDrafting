...
interface UseDraftSyncResult {
  ...
  onDraftAction: (handler: (action: DraftActionPayload, senderId: string) => void) => () => void;
}
...
export function useDraftSync(
  ...
  const [draftState, setDraftState] = useState<DraftState>(() => 
    roomState?.draftState || {
      phase: "map-pick",
      currentTeam: "team1",
      team1: { pickedUmas: [], bannedUmas: [], pickedMaps: [], bannedMaps: [] },
      team2: { pickedUmas: [], bannedUmas: [], pickedMaps: [], bannedMaps: [] },
      availableUmas: [],
      availableMaps: [],
      wildcardMap: {} as any,
    }
  ...
  const [lastValidatedChecksum, setLastValidatedChecksum] = useState<string>("");
  ...
  const draftActionHandlersRef = useRef<Set<(action: DraftActionPayload, senderId: string) => void>>(new Set());

  /**
   * Subscribe to incoming draft actions (host only)
   */
  const onDraftAction = useCallback((handler: (action: DraftActionPayload, senderId: string) => void): (() => void) => {
   ...
   */
  const broadcastState = useCallback((state: DraftState, currentVersion: number) => {
    const connections = getConnections();
    console.log("broadcastState called, connections:", connections.length, "peerId:", peerId);
    if (!peerId || connections.length === 0) {
      console.log("broadcastState early return - no peerId or no connections");
      return;
    }
   ...
   * Sends a draft action to the host (non-host only)
   */
  const sendDraftAction = useCallback((action: DraftActionPayload) => {
    if (isHost) {
    ...
    const messageType = action.itemType === "uma" 
      ? "select-uma" 
      : "select-map";
   ...
   */
  const handleStateSyncMessage = useCallback((message: NetworkMessage) => {
    if (isHost) {
    ...
    if (newVersion > version) {
      ...
      console.log("State synced from host, version:", newVersion, "checksum:", checksum);
   ...
   */
  const handleDraftActionMessage = useCallback((message: NetworkMessage) => {
    if (!isHost) {
    ...
    console.log("Received draft action from", message.senderId, message.payload);
